...
import { DesignPickPromo } from "@/components/DesignPickPromo";
...
export function HomeClient({ data }: { data: HomeData }) {
  ...
  const trust = [
    ...
    { icon: Wallet, title: t("trust.kaspi_t"), text: t("trust.kaspi_s"), kaspi: true },
    ...
    <div className="pb-8">
      ...
      <section className="max-w-7xl mx-auto px-4 pt-5">
        <div className="rounded-2xl md:rounded-3xl bg-gradient-to-r from-brand/[0.09] via-brand/[0.05] to-transparent ring-1 ring-brand/15 px-2 py-1.5 md:px-4 md:py-2">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 lg:divide-x divide-brand/15">
            {trust.map((b) => (
              <div key={b.title} className="flex items-center gap-3.5 px-3 py-3 md:px-5 md:py-4">
                {b.kaspi ? (
                  ...
                  <img
                    src="/kaspi-icon.png"
                    alt="Kaspi"
                    className="w-11 h-11 shrink-0 drop-shadow-[0_6px_16px_rgba(241,70,53,0.45)]"
                  />
                ) : (
                ...
                <div className="min-w-0">
                  ...
                  {b.kaspi ? (
                    <div className="mt-1 flex items-center gap-1.5 flex-wrap">
                      <span className="rounded-md bg-[#f14635]/10 text-[#f14635] px-1.5 py-0.5 text-[12px] font-bold">0-0-12</span>
                      <span className="rounded-md bg-[#f14635]/10 text-[#f14635] px-1.5 py-0.5 text-[12px] font-bold">0-0-24</span>
                    </div>
                  ) : (
